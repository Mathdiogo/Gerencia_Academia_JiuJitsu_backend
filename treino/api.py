...
from .models import Alunos, Aulas_concluidas
...
def progresso_aluno(request, email_aluno: str):
    ...
    n = order_belt.get(faixa_atual)  #order_belt é o dicionario de graduacao.py das faixas
    total_aulas_proxima_faixa =  calculate_lessons_to_upgrade(n)
    total_aulas_concluidas_faixa_atual = Aulas_concluidas.objects.filter(aluno=aluno, faixa_atual=aluno.faixa).count()
    aulas_faltantes = total_aulas_proxima_faixa - total_aulas_concluidas_faixa_atual
...
def update_aluno(request, aluno_id:int, aluno_data: AlunosSchema):
    ...
    for attr, value in aluno_data.dict().items(): #attr = atributo
        if value:
            setattr(aluno, attr, value) #Da instancia dessa classe (aluno), altere o atriburo (attr), para este valor (value) os 3 atributos de setattr
